...
<video controls className="w-full rounded-xl mb-4" src={`http://localhost:5000${video.videoUrl}`} /> <h1 className="text-2xl font-semibold mb-2">{video.title}</h1> <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm text-gray-400 mb-3"> <p>{video.channel}</p> <p>{formatViews(video.views)} views • {new Date(video.created_at).toLocaleDateString()}</p> </div>

      <div className="flex items-center gap-3 mb-5 flex-wrap">
        <button onClick={() => handleReaction("like")} className={`flex items-center gap-2 px-4 py-2 rounded-full transition ${userAction === "like" ? "bg-gray-700 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}>
          <ThumbsUp size={18}/> {likes}
        </button>
        <button onClick={() => handleReaction("dislike")} className={`flex items-center gap-2 px-4 py-2 rounded-full transition ${userAction === "dislike" ? "bg-gray-700 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}>
          <ThumbsDown size={18}/> {dislikes}
        </button>
        <button onClick={handleShare} className="flex items-center gap-2 px-4 py-2 rounded-full bg-gray-800 text-gray-300 hover:bg-gray-700 transition">
...
} catch (err) { console.error("Error fetching comment likes:", err); }
};

const handleReaction = async (type) => {
if (!userId) return alert("Please login to react.");
try {
await axios.post("[http://localhost:5000/api/comment/like](http://localhost:5000/api/comment/like)", { user_id: userId, comment_id: comment.id, type });
if (type === "like") { if (userReaction === "like") { setLikes(likes - 1); setUserReaction(null); } else { setLikes(likes + 1); if (userReaction === "dislike") setDislikes(dislikes - 1); setUserReaction("like"); } }
else { if (userReaction === "dislike") { setDislikes(dislikes - 1); setUserReaction(null); } else { setDislikes(dislikes + 1); if (userReaction === "like") setLikes(likes - 1); setUserReaction("dislike"); } }
} catch (err) { console.error("Error reacting:", err); }
};

return ( <div className="mb-4 border-b border-gray-700 pb-2"> <p className="text-sm font-medium text-gray-200">{comment.username}</p> <p className="text-gray-400">{comment.content}</p> <div className="flex gap-3 mt-1 text-gray-400 text-sm">
<button onClick={() => handleReaction("like")} className={`flex items-center gap-1 hover:text-green-500 transition ${userReaction==="like"?"text-green-500":""}`}><ThumbsUp size={16}/> {likes}</button>
<button onClick={() => handleReaction("dislike")} className={`flex items-center gap-1 hover:text-red-500 transition ${userReaction==="dislike"?"text-red-500":""}`}><ThumbsDown size={16}/> {dislikes}</button> </div> </div>
);
};
